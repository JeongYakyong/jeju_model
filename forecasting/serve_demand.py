"""2-B. 제주 수요 D+1~D+5 장지평 서빙 — DB 전용 (직접 다지평 LGBM, 2-A).
...
input_data_jeju.db 한 곳에서 읽고 쓴다.
  - 입력 : historical (실측 real_demand_jeju + 3지점 기상),
           ...
           refdata/jeju_ppa_btm_capacity_mw.csv (월별 BTM/PPA 용량)
  - 모델 : models/demand/lgbm_jeju_demand_direct.txt  (2-A, h=1..120, quantile α=0.60)
...
직접 다지평이라 **원하는 지평만큼의 예보만 있으면 된다**:
  24h 예보 → D+1, 120h 예보 → D+5. lag168·rec 는 과거 실측에서 와서 사슬이 없다.
  과거 실측 수요는 원점 직전 ~168h(7일) 필요(lag168·rec168).

2026-07-30 재학습으로 모델 지평이 168h(D+7) → **120h(D+5)** 로 줄었다. 예보 수집이
`--days 5` 라 D+6/7 은 서빙 입력이 기후값 폴백뿐이었고, 운영 지평(serve_chain.HZ=1..5,
pages/common.JEJU_HZ_MAX=5)도 이미 5일이다. days_ahead 가드도 1~5 로 함께 좁혔다.
...
공개 API
================================================================================
    predict_demand_to_db(origin_date, days_ahead=5, write=True)
    backfill_demand_to_db(start, end, days_ahead=5)

(원본: forecastmodel/02_jeju_demand_forecaster/serve_jeju_demand_lh.py)
"""
...
DB_PATH = P.DB_JEJU
MODEL   = os.path.join(P.DIR_MODELS_DEMAND, 'lgbm_jeju_demand_direct.txt')
CAP_CSV = P.REF_JEJU_PPA_BTM   # 제주 BTM·PPA 태양광 용량(월별) — data/refdata/
...
def load_target_weather(targets: pd.DatetimeIndex, clim: dict) -> pd.DataFrame:
    ...
    # 구름 4종 (raw west/south): forecast 우선(3h 구멍은 위와 같이 보간), 없으면 기후값
    for c in CLOUD:
...
def predict_demand_to_db(origin_date: str | None = None, days_ahead: int = 5,
                         write: bool = True, verbose: bool = True) -> pd.DataFrame:
    if not (1 <= days_ahead <= 5):
        raise ValueError('days_ahead 는 1~5 (모델 학습 지평 1..120h)')
...
# =============================================================================
def backfill_demand_to_db(start: str, end: str, days_ahead: int = 5,
...
if __name__ == '__main__':
    ...
    try:
        ...
        pass
    p = argparse.ArgumentParser(description='제주 수요 D+1~D+5 장지평 서빙 (직접 다지평 LGBM, 2-A)')
    ...
    pp.add_argument('--days', type=int, default=5, help='며칠 앞까지 (1~5, 기본 5)')
    ...
    bf.add_argument('--days', type=int, default=5)
