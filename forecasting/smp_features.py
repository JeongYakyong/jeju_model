...
같은 규칙으로 만든다. smp_binary.py(학습)·smp_d1.py(서빙)가 import.
...
# 원본(forecastmodel)은 training/ 하위폴더를 sys.path 에 올리는 방식이었으나,
# jeju_model 은 SMP 학습·서빙 파일을 forecasting/ 한 폴더에 두므로 일반 import 로 충분하다.
DB_PATH = P.DB_JEJU
...
# D+2 는 DA 잔차회귀(smp_da, TARGET='smp_jeju_da').  rt 점예측은 잠긴 실패경로
