...
무엇을 하나 (전부 D+2 전용 신규 컬럼, D+1 A안/Phase2 산출물 불변)
  ① 가격선 : smp_da.smp_d2_da.pkl 로 D+2 DA 예측 → est_smp_jeju_d2
  ② 음수경보: models/smp/smp_binary.pkl(D+1 분류기 그대로) 재사용.
             입력만 D+2 예측값으로 — 가격선 피처 smp_jeju_da = 위 예측 DA로 치환.
             → smp_neg_proba_d2, smp_danger_d2 (θ + 2h 지속)
  ③ 깊이   : models/smp/smp_depth_lookup.json(Phase2) 그대로 → smp_neg_depth_d2_p10/50/90
...
from forecasting.smp_features import DB_PATH, FEATURES as CLF_FEATURES, load_forecast
from forecasting.smp_binary import persist, NEG_THRESH
from forecasting.smp_da import FEATURES as DA_FEATURES, _predict_da
from forecasting.smp_depth import lookup_depth

MODELS = P.DIR_MODELS_SMP
