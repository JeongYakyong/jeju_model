...
input_data_jeju.db 한 곳에서 읽고 쓴다.
  ...
  - 모델 : models/smp/smp_binary.pkl (이진 음수분류기 + θ + 2시간 지속규칙)
...
피처빌더는 학습과 동일 파이프(smp_features.load_forecast)를 재사용 → train/serve parity 보장.
...
from forecasting.smp_features import DB_PATH, FEATURES, load_forecast
from forecasting.smp_binary import persist

BUNDLE = os.path.join(P.DIR_MODELS_SMP, 'smp_binary.pkl')
...
def _load_model(which='p25'):
    ...
    th = b.get('theta')
    if th is None:
        raise ValueError('번들에 θ가 없음 — smp_binary 후 운영점 저장 필요')
