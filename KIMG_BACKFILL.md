...
~~대신 **시각별 편향 보정**이 유망하다~~ → **2026-08-21 종결: 보정은 도입하지 않는다.**
사용자 결정 — robust·솔직 우선, 오차는 그대로 둔다.  다시 제안하지 말 것.
재측정 결과 보정 이득 자체는 실재하고 오히려 더 컸으나(완전 홀드아웃 기온·습도·풍속
−17~22%, 롤링 60일이면 최대 −22%), **"보정 후엔 KIMR 이 KIMG 를 이긴다"는 철회됐다** —
맞물린 표본으로 재니 격차만 5.2%→0.3% 로 좁혀지고 뒤집히지 않았다.
상세는 `DECISIONS.md` 2026-08-21.
...
~~★**결정은 보류한다** — 시각 보정을 하면 기온 우열이 뒤집히므로~~
→ **2026-08-21 정정: 뒤집히지 않는다.**  보정 유무와 무관하게 **기온·습도는 KIMG,
풍속은 KIMR** 이 우세하다.  보정도 도입하지 않기로 했으므로, 소스 구조 결정이
무언가를 기다릴 이유는 없다 — 위 표만 보고 정하면 된다.
