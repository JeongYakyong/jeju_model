...
### 사용자 결정 사항 (재질문 금지)
- **재학습한다. 단 이걸로 문제해결을 장담하지 않는다** (사용자 명시).
- 바꾸는 것은 **`Year_sin`/`Year_cos` 추가**와 **val 4계절 재분할** 둘뿐.
- **cloud_scale 후처리는 재학습 뒤에 판단** — 지금 파라미터를 맞춰 봐야 곧 교체될 모델
  기준이라 헛일이다.

### ★핵심 — 같은 숫자가 같은 뜻이 아니다
상세 = `Training/3_jeju_solarwind_forecaster/REPORT_cloud_feature_audit.md`.

예보 `tcld` 는 권운을 **100% 반영**(0.99)하는데 ASOS 전운량은 **45%만**(0.44) 센다.
학습은 ASOS 축, 서빙은 `tcld` 축 → 예보 total=1.0 일 때 실제 su 0.324, 실측 total=1.0
일 때 0.209.  **−0.115** 로 측정된 편향 −0.110 과 일치한다.

- `tcld` 는 여름에 권운 지표로 변한다: 8월 r(total,high) 0.879 / r(total,low) 0.201,
  `total=1.0` 일 때 low 평균 **0.232**.  겨울·봄엔 반대(low 0.57~0.83 = 진짜 overcast).
- 층별 최적 가중 **LCLD 1.00 : MCLD 0.70 : HCLD 0.24~0.37** 인데 `tcld` 는 동등 결합.
- `tcldc`(대류운) 는 **97.9%가 0** — 쓰면 안 된다.  운량 5변수 전부 `LEVEL=0`.

### ★실패로 확인된 것 셋 (전부 역방향 분할·LOMO 에서 무너짐)
| 후보 | 단방향 홀드아웃 | 역방향/LOMO |
|---|---|---|
| 투과가중 지표 | 단변량 대조 최고 | 회귀에 넣으면 이점 소멸(맑음 −0.094) |
| `total_cloud` 제거(ⓐ) | 맑음 −0.079→−0.061 | LOMO 에서 현행보다 나쁨(0.1135 vs 0.1102) |
| cloud_scale 후처리 | MAE 최고 0.1014 | 역방향 0.1535 vs 현행 0.1234, floor 가 격자 경계 |

**운량은 노이즈가 아니라 흐린날 전용 신호다.**  빼면 여름 맑음은 −0.011 로 좋아지나
흐림 +0.119(3배)·과대율 65% = 업무 위험 방향이고, 2월 맑음이 −0.248 로 무너진다.

### ★재학습 실행 결과 — 실패, 되돌림 (오후)
실서빙 A/B (81 base / 낮 3,160행 / 구 모델을 git 복원해 같은 solar_scale·예보 입력으로 재실행):

| | MAE | 맑음 | 흐림 | 과대율 |
|---|---|---|---|---|
| 구 (07-30) | **0.1323** | **−0.0986** | 0.0415 | 40.2% |
| 신 (08-25) | 0.1381 (+4.4%) | −0.1220 (+23.7%) | **0.0343** | **38.4%** |

가설 = split 재설계로 train 13개월(2025년) 상실.  가중치는 `git checkout` 으로 되돌렸고
**신 가중치는 백업 못 해 소실**(Colab 재취득 필요).

### ★예보 축 직접 학습도 실패 (LGBM 5변형)
맑음 편향 −0.135~−0.273 vs 현행 −0.101.  예보 아카이브가 6개월뿐이라 여름 외삽 불가.
`lwdown`(광학두께 대리)은 물리적으로 타당하나 표본이 얇아 과적합만 키웠다.
**재시도 조건 = 층별·lwdown 18개월 축적(2027-08경).**

### ★일사 "평균 일치"는 착시였다 — 선행 리포트 정정
결과 기반 기준(그날 최대 이용률): 흐림 **+49.1%** / 맑음 −5.1%(실질 −3%), 무조건부 +6.8% 과대.
흐린 날은 **운량은 맞히는데 일사를 +49% 과대** — 운량은 면적이지 광학 두께가 아니다.
편향 분해: 맑음 = 일사 44% + 운량 56%,  MAE = 일사 **86%** + 운량 14%.
**예보 품질 천장**: 같은 모델에 입력만 바꿔 MAE 실측 0.0764 vs 예보 0.1125(+47%).

### 재학습 코드 (완료)
`training/_gen_notebook_solar_d1d5.py` 수정 → `train_solar_d1d5_colab.ipynb` 재생성.

- `future_features_solar` 10개 → **12개** (`Year_sin`/`Year_cos` 를 **뒤에만** 추가,
  앞 10개 순서 보존 확인).  `features_solar` 13 → 모델 `num_features` 13.
- split: train 2020-01~2024-12(43,848행) / **val 2025 전체(8,760행, 12개월)** /
  test 2026-01~끝(5,664행).  `TEST_END=None` 이면 데이터 끝까지 자동.
- **val 이 12개월을 담는지 `assert`** 로 못 박았다 — 계절 편향 재발 방지.
- 로컬 dry-run(데이터 준비 셀만 실행)으로 전부 통과 확인.  GPU 불필요.
- **서빙은 무수정** — `_build_solar_direct` 가 `_add_time_feats` 로 Year 를 이미 만들고,
  `_infer` 는 `md['future_features_solar']` 로 열을 고르며 `num_features` 도 metadata
  에서 나온다.  새 가중치·스케일러·metadata 만 넣으면 된다.

### 그 밖에 확인된 것
- **MinMax 스케일러 무혐의** — max 3.86 은 매끄러운 꼬리의 끝(상위 15개 3.70~3.86,
  전부 5~8월 정오), 학습창 전용 적합(누수 없음), 선형이라 신경망이 흡수.
  다만 예보는 스케일 공간 상위 7~9% 를 안 밟는다(P99 에선 0.850 vs 0.847 로 일치).
- **KIMR 등압면은 ASOS 전운량과 상관이 오히려 높다**(west r 0.632 > KIMG 0.591).
  분포는 과잉 맑음(=0 이 47.6%)이지만 타이밍 정보는 더 있다 — 08-24 평가를 보정.
- **예보 상층운 과예보 +49%**(7월 +0.432).  단 ASOS 미탐지와 섞여 있어 **상한**이다.

---

## 2026-08-24 (오후) — 태양광 맑은날 과소예측 원인 규명 + 수집 속도 병목 해소

### 사용자 결정 사항 (재질문 금지)
- **다음 세션 주제 = 태양광 재학습.** 이번 세션은 **원인 파악까지**가 목표였고 달성했다.
- 예보 기반 전면 학습은 **불가**(트랜스포머에 8~9개월은 부족). 파인튜닝은 검토 대상.

### ★태양광 과소예측 = 예보 **운량**의 분포 어긋남 (모델 결함 아님)
상세는 `Training/3_jeju_solarwind_forecaster/REPORT_solar_clearsky_underprediction.md`.

핵심 하나만: 실제로 밝았던 시각(실측 이용률>0.4) 211개를 예보 운량으로 가르면 —

| 예보 운량 | 표본 | 편향 |
|---|---|---|
| <0.5 (맑다고 봄) | 18 | **+0.001** |
| ≥0.95 (흐리다고 봄) | 126 | **−0.110** |

**예보가 맑다고 맞히면 모델은 정확하다.**  그런데 밝았던 시각의 60%에 예보 운량이
≥0.95 가 붙는다(그 시각 실측 운량 평균 0.504 vs 예보 0.864).

- 운량 분포: 예보 평균 **+34%**, P25 **+130%**, 맑음(<0.3) 비율 24.1%→11.2%.
- 일사 분포: 평균은 거의 일치, **최대만 −12.5%** — 운량 어긋남이 훨씬 크다.
- 소거된 용의자: `solar_scale`(60%+ 구간 scale 0.994 로 거의 미작동) / 스케일러
  (MinMax·clip=False·타깃 미스케일) / capacity(04-24 이후 455.0 고정, 변동 0%).
- **초기 모델(south 일사 단독)이 맑은날 좋고 흐린날 나빴던 것과 정확히 반대**인 이유:
  일사는 분포가 맞고 운량은 안 맞는다. 운량을 넣어 흐린날을 얻고 맑은날을 잃은 교환이다.

⚠**파인튜닝 표본 수 착각 주의**: `SEQ_LEN=336`·`PRED_LEN=24` 라 **base 1개 = 지평당 창 1개**.
행 21,980 이 아니라 **지평당 245창 / 전체 1,225창**이다.

### 수집이 6.4분 걸리던 원인 = KMA API 장애 (코드 아님)
`apihub.kma.go.kr` 이 응답을 안 주고 있었다. 단일 호출 실측:

| | 장애 중 | 회복 후 |
|---|---|---|
| KIMR 단일면(met) | 69.2s **실패** | **0.56s OK** |
| KIMG(22변수) | 51.5s **실패** | **0.45s OK** |
| KIMR 등압면 CLDFRA | 69.2s **실패** | **0.89s OK** |

`fetch_pt_std` 가 30초 타임아웃 × 2회 + 백오프라 실패 1건에 69초를 쓴다.
**변수·엔드포인트 문제가 아니다** — `T2` 하나만 요청해도 똑같이 죽었다.
★**아카이브 커버리지 30%/0% 도 `--point-workers 3` 탓이 아니라 이 장애였다.**
회복 후 같은 3워커로 12 base 연속 **커버리지 100% / 실패 0**.

### 코드 변경
- `kma_kimr_nc.fetch_nc_long` 에 **지점 병렬**(`point_workers`) + **hf 20개마다 진행률**
  (`NC_PROGRESS_EVERY`, 누적 성공수·초당 hf, `flush=True`).
  → 병렬화하면 지점 완료까지 무음이 되므로 진행률이 **더** 필요해졌다.
- `collect_forecast` 에 `--point-workers`(기본 3) 신설·관통.
  실측: 같은 base 재수집 **6.4분 → 1분 52초**, KIMR 3지점 각 ~10초, 커버리지 119/119.
- `postprocess`: `temp_skin` 급변 한계를 기온과 분리(5.0→12.0), 접두사 선택을 최장 일치로.
  매 base 뜨던 오경보(rc=1) 해소. 정상 4 base 경고 0, 합성 손상은 그대로 탐지.

### 데이터 상태 — 전부 최신
`forecast_horizon` 245 / `est_horizon_jeju` 245 / `est_smp_horizon_jeju` 245 (~08-23),
`forecast_kimr` 187 · `forecast_kimg` 249 (~08-23), `historical` ~08-24 23:00.
예보↔예측↔SMP 사이 구멍 0.  `--verify` rc=0.  AppTest exception 0.

08-18 서빙체인 검증(재수집→체인→실측대조): D+1~D+5 각 24행 NULL 0,
수요 MAPE 2.34% / 순부하 nMAE 8.15% / 태양광 capMAE 4.61% / 풍력 capMAE 7.71%.

---

## 2026-08-24 — 서빙 소스 구조 확정 + 좌표 의혹 규명 + 예측 3주치 복구

### 사용자 결정 사항 (재질문 금지)
- ★**서빙 met 소스 구조는 현행 유지로 확정** — `kimr_part.combine_first(kimg_part)` 그대로.
  3주간 열려 있던 항목 종결.
- "서빙체인 교체" = **예측 재생성**을 뜻한 것. 서빙 입력을 소스 분리 테이블로 옮기는
  경로 이관이 아니다.

### 소스 결정 — **모델이 먹는 형태로 재야 답이 나온다**
08-12 은 지점별로 따로 쟀는데, 수요 모델은 3지점을 **평균**해 먹는다. 평균을 내면
지점 오차가 상쇄돼 KIMG 우위가 모델 입력까지 살아남지 못한다.

| 입력 | 쓰는 곳 | KIMR | KIMG | 격차 | 월 일관성 |
|---|---|---|---|---|---|
| 기온 3지점평균 | 수요 | 1.1915 | 1.1639 | KIMG 2.3% | **KIMG 3/7** |
| 습도 3지점평균 | 수요 | 8.4490 | 8.1739 | KIMG 3.3% | KIMG 4/7 |
| **풍속 west** | 풍력 | **2.2363** | 2.4533 | **KIMR 8.8%** | **KIMR 7/7** |
| **풍속 east** | 풍력 | **1.5814** | 1.8297 | **KIMR 13.6%** | **KIMR 6/7** |

기온·습도는 월별로 KIMR 이 오히려 4승 3패라 **노이즈**다. 풍속 KIMR 우위는 진짜인데
**이미 KIMR** 이라 바꿀 것이 없다. 변수별 분리는 `src_met_<지점>` 이 지점당 출처 하나만
담는 구조라 **출처 표시가 거짓이 된다** — 2% 얻자고 오차 추적을 잃는다.

### ★west 수집 좌표는 바다가 맞다. 그러나 원인이 아니다
사용자가 "격자/위경도를 바다로 지정한 건 아니냐"고 물어 확인했다.
`data/refdata/jeju_emd_2013.json` point-in-polygon:

| 지점 | 판정 |
|---|---|
| 예보 West(Gosan) 33.4427,126.1713 | **바다 — 육지까지 8.5km** (고산 ASOS 보다 16km 북쪽) |
| 예보 East / south | 육지 |

`fetch_pt_std` 문서에 "x/y 격자 고정 = **제주 바다 셀 회피**"라고 적혀 있어 이미 알려진
문제였다. **그런데 좌표를 고산 ASOS 로 고쳐 같은 base 를 받아 보니 진폭이 안 살아난다**
(현행 격자 2.05K → 고산 위경도 1.96K, 대조군 east 4.83K).
고산은 좁은 곶이라 R030 격자에선 **어느 셀을 잡아도 바다가 지배적**이다.
→ **좌표는 건드리지 않는다. 같은 의심을 반복하지 말 것.**
(단 GeoJSON 이 거칠어 오차 ~2km — 실제 육지인 고산 ASOS 도 "바다 1.9km"로 나온다.
 8.5km 는 그 오차로 설명되지 않는다.)

### ★일변화 압축은 NWP 일반 특성이 아니라 **지점 문제**였다
| 지점 | 실측 진폭 | KIMR | KIMG | KIMR비 |
|---|---|---|---|---|
| west | 3.35 | 0.90 | 1.13 | 27% |
| **east** | 4.65 | **4.36** | 3.32 | **94%** |
| south | 4.64 | 1.42 | 2.83 | 31% |

**east 는 94% 를 재현한다 — 압축이 사실상 없다.** 08-12 의 "제주가 작은 섬이라서"는
west·south 에만 해당한다. 좌표가 ASOS 와 정확히 일치하는 지점이 곧 정상인 지점이다.

### 건전성 경고 4건 = 임계값 문제 (서빙 무영향)
전부 `temp_skin_east` 1h 급변(>5°C). 실제로는 일몰 19시 28.3→23.0, 정오 13시 28.3→34.5 로
**물리적으로 정상**이다. 임계값이 기온용이라 지표온에 걸린 것.
`temp_skin` 은 `forecasting/`·`pages/` 참조 **0건**이라 서빙에 영향 없다.
- ⚠ 오히려 **서·남부 지표온이 얼어붙어 있다** — 1h 변화 평균 west 0.014 / south 0.033 vs
  east 1.15 (최근 14 base 일관). **재학습에서 지표온을 피처로 쓰면 서·남부는 못 쓴다.**
- ⚠ 이대로 두면 매일 rc=1 이 뜬다(경보 피로). 임계값 조정은 미결.

### 데이터·예측 복구 완료
- `historical` 58,000+행 / 08-24 23:00. `*_da` 결손 전부 복구.
  **07-31 `smp_jeju_da` 24시간 결측**이 남아 있어 base 08-05 의 D+2 가 건너뛰어졌다
  (문서대로 lag168 의존) → 백필 후 재실행으로 해소.
- `forecast_horizon` **245 base 전부 완전**(`--verify` rc=0). 불완전했던 08-04 재수집.
- `est_horizon_jeju` 08-03 → **245 base / 08-23**, `est_smp_horizon_jeju` 245 base.
  3주 만에 현재 시점을 따라잡았다.
- AppTest 렌더 exception 0.
- 아직 안 함: **②-2 `collect_archive`** (`forecast_kimr` 07-29 / `kimg` 08-11 에서 정지).
  서빙은 안 읽으므로 급하지 않으나 **API 보존 80일/160일** 이 걸린다.

---

## 2026-08-21 — 복귀 정비 + 시각별 편향 보정 **종결**(도입 안 함)

9일 만의 복귀. 앞 세션이 문서 분리를 하다 만 상태였고, 데이터가 17일 밀려 있었다.

### 사용자 결정 사항 (재질문 금지)
- ★**시각별 편향 보정은 도입하지 않는다.** "robust 하고 성능보다 솔직한 모델을 아주
  선호한다. 오차는 그냥 두자." — 이득이 실재해도 넣지 않는다. **다시 제안하지 말 것.**

### 08-12 의 핵심 주장이 재현되지 않았다
저장된 스크립트가 없어 새로 짜서 쟀다(완전 홀드아웃: 적합 2~5월 / 홀드아웃 6~7월).

| | 08-12 기록 | 재측정 |
|---|---|---|
| 실측 주야 진폭 | 3.36°C | **4.27°C** |
| KIMR / KIMG 진폭 | 0.84 / 1.28 | **2.13 / 2.34** |
| 기온 MAE 개선 KIMR / KIMG | −13.5% / −2.7% | **−16.5% / −11.8%** |

★**"보정 후엔 KIMR 이 KIMG 를 이긴다" = 철회.** 08-12 은 두 소스를 **서로 다른 행
집합**(150 vs 185 base)에서 쟀다. 같은 base·시각·지평으로 3-way join 해 맞물리면
격차가 5.2%→0.3% 로 좁혀질 뿐 **뒤집히지 않는다**(KIMG 1.1232 vs KIMR 1.1270).
→ 보정 유무와 무관하게 **기온·습도는 KIMG, 풍속은 KIMR**. 소스 구조 결정이 보정을
기다릴 이유가 없어졌다.

### 보정을 안 쓰기로 했지만, 재어 둔 것은 남긴다
- **변수마다 필요한 자유도가 다르다.** 풍속은 지점 상수 3개로 이득이 **전부** 설명된다
  (−15.6% vs 시각별 −15.7%). 기온·습도는 반대로 시각 성분이 핵심(−10.2%p, −7.8%p 추가).
  KIMG 풍속은 진폭비가 102%(압축 없음)인데도 보정이 먹혔다 → 이득의 출처가 진폭이
  아니라 **수준 편향**인 경우가 있다.
- **고정 보정표보다 롤링 재적합이 낫다** — 직전 60일 롤링이 거의 전 조합에서 우세
  (기온 KIMG −11.8%→−21.5%). 08-12 의 "고정 표는 드리프트에 취약"이 실증됐다.

### 버그 하나 — KPX `*_da` 가 하루 66행을 준다
API 가 1~9시를 `'01'` 과 `'1'` 두 형식으로 각각 발행한다(48행 → 66행). 값은 `rn` 만
다른 완전 동일 행. `int(hour)-1` 매핑에서 같은 시각 두 행이 되고 `merge` 가 못 합쳐
`build_historical` 의 concat 이 `InvalidIndexError` 로 **죽는다**.
→ `_fetch_kpx_est_one_day` 에서 `(date, hour, areaName)` dedup. `fetch_kpx_jeju`·
`fetch_asos` 는 무관하다.

### 데이터 복구
- **실측 백필 완료** — 58,195행(+462), 최신 08-21 18:00.
- ⚠**`*_da` 는 08-08~08-21 이 비어 있다** — 진단 호출로 **KPX 일일 요청제한을 소진**했다.
  자정 리셋 후 `collect_historical --backfill 20` 재실행하면 채워진다(UPSERT 라 안전).
  **쿼터는 서비스별이다** — `SmpWithForecastDemand` 만 막히고 수급·RT SMP 는 정상이었다.
- **예보 백필은 진행 중** — `collect_forecast --backfill 19`, **base 당 약 6분**(12z 5일,
  지점 3 × 120시각 순차)이라 18개에 2시간 안팎. resume-skip 이라 끊겨도 안전.
- 아직 안 함: **②-2 `collect_archive`** (forecast_kimr 23 base / kimg 10 base 결손).
  규약대로 ② 가 끝난 뒤에 돌린다.

### 그 밖
- 앞 세션이 하다 만 **수집 API 상세 → `collectors/CLAUDE.md` 분리**를 커밋했다
  (삭제 79줄 전부 대조, 손실 0). 루트에서 지운 개인 설정 절은 글로벌 CLAUDE.md 와 중복.
- **git push 는 여전히 미실행** — 로컬 커밋이 리모트 `Initial commit` 위에 쌓여 있다.
...
- **시각 보정만으로 KIMR 기온 MAE −13.5%**(KIMG −2.7%), 적합 2~5월/홀드아웃 6~7월.
  ...
  ⚠ **2026-08-21 재현 실패 — 이 두 줄의 결론은 철회됐다.** 맞물린 표본으로 재니
  뒤집히지 않는다(아래 08-21 항목). 보정 자체도 도입하지 않기로 했다.
