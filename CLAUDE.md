...
## 언어
- 한국어로 대화하되, 쉽고 자연스러운 한국어를 사용할 것
- 번역하면 어색해지는 기술 용어나 표현은 무리하게 한국어로 바꾸지 말고 영어 그대로 쓸 것 (예: feature engineering, refactoring)

## 문제 해결 방식
- 예상치 못한 장애물을 만나면 (파일이 열려 있어 수정 불가, 권한 문제 등) 복잡한 우회 코드를 짜지 말 것
- 대신 먼저 상황을 사용자에게 알리고 함께 해결 방법을 정할 것
- 간단한 해결책이 있는지 항상 먼저 확인할 것

## 의사결정
- 다음 결정은 절대 혼자 하지 말고, 옵션과 이유를 설명한 뒤 사용자 확인을 받을 것:
  - 데이터셋 피처 선택/제거
  - 데이터 삭제 또는 변형
  - 분석 방법론이나 모델 선택
- 작업 진행 중 단계별로 무엇을 왜 하는지 간단히 설명할 것

## 코드 스타일
- 사람이 읽기 쉬운 코드를 최우선으로 할 것
- 변수명을 과도하게 축약하지 말 것 (나쁜 예: df_test → d_t)
- 이름만 봐도 의미를 알 수 있는 변수명 사용 (예: test_scores, user_count)
- 짧고 영리한 코드보다 명확하고 단순한 코드를 선호할 것
